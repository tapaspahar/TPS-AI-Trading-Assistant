"""Dhan read-only live price stream using rate-safe quote polling."""
...
class DhanStream:
    ...
    def start(self, exchange_type, token):
        if not getattr(self.client, "session", None):
            raise RuntimeError("Connect Dhan before starting a market feed.")
        exchange = self.EXCHANGE_BY_TYPE.get(int(exchange_type))
        if not exchange:
            raise RuntimeError("This Dhan live-feed exchange is unsupported.")
        self._stop.clear()

        def run():
            self.on_status("Dhan live feed connected")
            while not self._stop.wait(1.1):
                try:
                    quote = self.client.get_option_quote(exchange, str(token))
                    self.on_tick({
                        "token": str(token),
                        "last_traded_price": int(float(quote.get("ltp", 0)) * 100),
                        "ltp": float(quote.get("ltp", 0)),
                    })
                except RuntimeError as error:
                    self.on_status(f"Dhan live feed issue: {error}")
            self.on_status("Dhan live feed disconnected")
