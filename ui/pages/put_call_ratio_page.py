...
class PutCallRatioPage(QWidget):
    ...
    def __init__(self):
        ...
        layout.setSpacing(9)
        title = QLabel("Put-Call Ratio & OI Observer")
        title.setObjectName("pageTitle")
        layout.addWidget(title)
        note = QLabel(
            "Live Call OI, Put OI, their change from the previous saved observation, OI-PCR and Volume-PCR are combined into an explainable market-sentiment context. OI alone never authorizes a trade."
        )
        ...
        self.status = QLabel("Automatic observer refreshes every 60 seconds while this page is open.")
    ...
    def show_result(self, result):
        ...
        self.explanation.setText(
            ...
            f"Caution: {'; '.join(sentiment['warnings'])}"
        )
        by_strike = {}
    ...
    def show_error(self, message):
        self.loading = False
        self.status.setText(f"PCR observer unavailable: {message}")
